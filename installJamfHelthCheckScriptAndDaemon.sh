...
#2026/04/02 - added validateJSSAvailable function - exit if cannot connect to the Jamf Server.
### Errors
# line 75 in validateJSSAvailable - doe not log on communication failure
# line 134 in main HERE script - does not exit and stop execution if validateJSSAvailable fails
...
validateJSSAvailable() {
    if [ ! /usr/bin/defaults read /Library/Preferences/com.jamfsoftware.jamf.plist jss_url ]; then
        log "Jamf plist is broken"
        exit 1
        fi 
    ...
    if [ "\$tomcat_chk" -eq 0 ]; then
           log "Machine can connect to \$jamfpro_server_address. Proceeding."
           exit 0
    else
           log "Machine cannot connect to \$jamfpro_server_address. Exiting." #!!! this does not log if connection fails
           exit 1
    fi
}
validateJamfDaemon() {
...
if (( TIME_DIFF < RUN_INTERVAL )); then
    ...
    # exit with failure if JSS not available
    #!!! this does not exit if JSS not available
    [ validateJSSAvailable ] || exit 1 
    validateJamfDaemon
    killJamfIfRunningMoreThan1Day
