...
# Note: We do NOT quote "GATOR" so that the variables above are expanded 
# into the script during creation.
...
validateJamfDaemon() {
    if /bin/launchctl list | grep -q "\$DAEMON_ID"; then
        log "CHECK: Jamf LaunchDaemon (\$DAEMON_ID) is active."
    else
        log "ALERT: Jamf LaunchDaemon (\$DAEMON_ID) is NOT loaded. Running jamf manage..."
...
killJamfIfRunningMoreThan1Day() {
    ...
    if [ -z "\$processRuntime" ]; then
        log "STATUS: Jamf binary runtime is under 24 hours."
    else
...
if ((TIME_DIFF >= $KILL_THRESHOLD)); then
    killJamfIfRunningMoreThan1Day
    echo "\$CURRENT_TIME" > "\$LAST_RUN_FILE"
else
    log "SKIP: 24-hour threshold not yet met."
fi
