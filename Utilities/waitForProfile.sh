...
#
####################################################################################################
if [[ "$4" != "" ]] && [[ $profileName == "" ]]; then
    profileName=$4
fi
if [[ "$profileName" == "" ]]; then
	echo "Input parameter not found: profileName"
	exit 1
fi
if [[ "$5" != "" ]] && [[ $maxTries == "" ]]; then
	maxTries=$5
...
if [[ "$7" != "" ]] && [[ $policyTriggerFail == "" ]]; then
	policyTriggerFail=$7
fi
if [[ "$policyTriggerFail" == "" ]]; then
	echo "Input parameter not found: policyTriggerFail"
	# exit 1
fi
#debug only
...
echo "Policy Trigger Fail: $policyTriggerFail"
###############################################
###  Functions
###############################################
function waitForProfileToExist() {
	...
	echo "profileName: $profileName" >> /Users/Shared/testing.log
	echo "profiles -P" >> /Users/Shared/testing.log
	echo $(profiles -P) >> /Users/Shared/testing.log
	echo "profiles -P | grep" >> /Users/Shared/testing.log
	echo $(profiles -P | grep "$profileName") >> /Users/Shared/testing.log
	...
	while [ $tries -lt $maxTries ]; do
	    ...
	    if sudo profiles -P | grep "$profileName"; then
	        echo "Configuration profile '$profileName' found. Continuing script." >> /Users/Shared/testing.log
	        echo "TRUE"
	        #break # Exit the loop if the profile is found
	        exit 0
	    else
	        echo "Configuration profile '$profileName' not yet found. Waiting..." >> /Users/Shared/testing.log
...
foo=$(waitForProfileToExist $profileName $maxTries)

if [[ $foo == TRUE ]]; then
	echo "Configuration profile found, run Success Policy"
	runPolicyList $policyTriggerSuccess
	exit 0
else
	echo "Configuration Profile not found, run fail policy"
	runPolicyList $policyTriggerFail
	exit 1
fi